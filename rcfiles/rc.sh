...
ADDPATHS="$HOME/secrets/scripts:$HOME/scripts:$HOME/.cargo/bin:$HOME/bin:/usr/local/cuda/bin:/usr/lib/go-1.8/bin:/usr/local/go/bin:$(echo "$GOPATH" | sed -e 's-:-/bin:-' -e 's-$-/bin-' )"
echo "$ADDPATHS" | tr ':' '\n' | while read x
do
  case ":$PATH:" in
    *":$x:"*) :;;
